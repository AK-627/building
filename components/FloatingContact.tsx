...
export default function FloatingContact({ contact }: FloatingContactProps) {
  const phone = contact?.phoneNumber ?? DEFAULT_PHONE;
  const message = contact?.whatsappMessage ?? DEFAULT_MESSAGE;

  return (
    <>
      {/* Desktop Floating Button (Rectangular) */}
      <div className="hidden md:flex fixed bottom-6 right-6 z-50 flex-col gap-3">
        <a
          href={buildWhatsAppUrl(phone, message)}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-3 bg-[#25D366] text-white px-5 py-3 rounded-xl shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group"
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" className="group-hover:scale-110 transition-transform">
          ...
          <span className="font-semibold text-[15px] tracking-wide">WhatsApp Us</span>
        </a>
      </div>

      {/* Mobile Sticky Bottom Bar */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-stone-200 flex shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
        <a
          href={buildTelUrl(phone)}
          className="flex-1 flex items-center justify-center gap-2 py-4 bg-navy text-white hover:bg-navy-light active:bg-navy-light transition-colors"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="flex-1 flex items-center justify-center gap-2 py-4 bg-[#25D366] text-white hover:bg-[#1ebe5d] active:bg-[#1ebe5d] transition-colors"
        >
