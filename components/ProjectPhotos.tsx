...
export default function ProjectPhotos({ photos }: ProjectPhotosProps) {
  ...
  const slides = photos.map((p) => ({ src: p.url, alt: p.alt || 'LODHA MIRABELLE' }));
      ...
      <div className="max-w-6xl mx-auto">
        <ScrollReveal>
          ...
          <p className="text-slate-500 text-center mb-10 text-sm uppercase tracking-widest">
            LODHA MIRABELLE — A Glimpse of Luxury
          </p>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {photos.map((photo, idx) => (
              <button
                ...
                aria-label={`View photo ${idx + 1}: ${photo.alt || 'LODHA MIRABELLE'}`}
              >
                <img
                  src={photo.url}
                  alt={photo.alt || 'LODHA MIRABELLE'}
